/* ========================================
   CORE JAVASCRIPT ENGINE V12 + FULL INTEGRATION
   PART 1/3 - Initialization & Authentication
   ======================================== */
...
/* ========================================
   PART 2/3 - User Management, Tag, Reply, Swipe
   ======================================== */
                    ...
                    <div class="member-item">
                        ...
                        <div class="member-info">
                            <div class="member-name">
                                ...
                                ${data.verified ? '<span class="member-verified">✅ VERIFIED</span>' : ''}
                            </div>
                            <div class="member-badge">${data.level || 'MEMBER'} • Bergabung: ${data.joinDate || '-'}</div>
                        </div>
                    </div>
                `;
            });
...
async function processMessageCommands(msg) {
    ...
    if(msg.startsWith('/v ') && me === 'rayy') {
        ...
        if(targetUser) {
            ...
            if(snap.exists()) {
                await userRef.update({ verified: true, level: 'verified' });
                await db.ref('messages').push({
                    user: 'SYSTEM',
...
/* ========================================
   PART 3/3 - Core Functions, Chat, Messages, Story, Events
   ======================================== */
...
function boot() {
    ...
    db.ref('settings').on('value', s => {
        ...
        if(d.roomStatus) {
            document.getElementById('room-status').innerHTML = `<i class="fas fa-circle" style="font-size: 8px;"></i> ${d.roomStatus}`;
        }
...
styleToast.textContent = `
    ...
    #toast-container { left: 50% !important; transform: translateX(-50%) !important; }
`;
